...
class CrossTransformer_mine(nn.Module):  #
    ...
    def forward(self, x, context, extract_feature=False):
...
class SegTextAsConditionHead(BaseDecodeHead):
    def __init__(
        self,
        text_categories,
        text_channels,
        ...
        **kwargs,
        ...
        super(SegTextAsConditionHead, self).__init__(**kwargs)
    ...
    def forward(self, batch):
        ...
        hw_shape = batch['query_feat'][-1][0].shape[-2:]
        x = inputs[-1]
        ...
        tmp = self.decoder_1(x, context=self.text_embeddings)
    ...
    def forward_multiscale(self, batch):
        inputs = batch['query_feat']
        hw_shape = batch['query_feat'][-1][0].shape[-2:]
        x = inputs[-1]
        q, k, v, cls_token = None, None, None, None
        ...
        if not multi_scale_feat:
            tmp = self.decoder_1(
                x,
                context=einops.repeat(
                    self.text_embeddings, 'c n -> b c n', b=feat.shape[0]
                ),
            )

            feat = tmp[:, 1:, :]
            feat = einops.rearrange(
                feat, 'b (h w) c -> b c h w', h=hw_shape[0], w=hw_shape[1]
            )

            output = self.cls_seg(feat)

            ## organize the 20 chaanel to 2 channel
            output = init_logits + output * 0.5
            ...
            gt_mask_20 = batch['query_mask'].long() * batch['class_id'].view(-1, 1, 1)
            gt_mask_20_onehot = F.one_hot(gt_mask_20, num_classes=20).permute(
                0, 3, 1, 2
            )
            bg_logits = output * (1 - gt_mask_20_onehot)
            bg_logits = bg_logits.sum(dim=1, keepdim=True)

            fg_logits = output * gt_mask_20_onehot
            fg_logits = fg_logits.sum(dim=1, keepdim=True)
            output = torch.cat([bg_logits, fg_logits], dim=1)

        # loss = self.loss_decode(
        #     output, batch['query_mask'].long() * batch['class_id'].view(-1, 1, 1)
        # )
        loss = self.loss_decode(output, batch['query_mask'].long())
    ...
    def forward_txt_as_classifier(self, batch):
        inputs = batch['query_feat']
        hw_shape = batch['query_feat'][-1][0].shape[-2:]
        x = inputs[-1]
        q, k, v, cls_token = None, None, None, None

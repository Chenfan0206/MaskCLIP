        ...
        if self.decode_mode == 'clip_context_affinity_v4':
            ...
            for _idx, A in enumerate(batch['affinity']):
                ...
                if _idx == 1:
                    ...
                    coarse_affinity_mask = torch.sigmoid(torch.einsum('bj,bjk->bk', mask,tmp))
                    _min = einops.reduce(coarse_affinity_mask, "b l -> b ()", "min")
                    _max = einops.reduce(coarse_affinity_mask, "b l -> b ()", "max")
                    coarse_affinity_mask = (coarse_affinity_mask - _min) / (_max - _min + 1e-8)
                    coarse_affinity_mask = coarse_affinity_mask.unsqueeze(1)

                    _size = int(A_ss.size()[-1] ** 0.5)
                    all_affinity_masks.append(einops.rearrange(coarse_affinity_mask, "b c (h w)-> b c h w", h=_size, w=_size))
                    ...
                    coarse_affinity_mask = torch.sigmoid(
                        torch.einsum('bj,bjk->bk', mask, tmp))
                    _min = einops.reduce(
                        coarse_affinity_mask, "b l -> b ()", "min")
                    _max = einops.reduce(
                        coarse_affinity_mask, "b l -> b ()", "max")
                    coarse_affinity_mask = (
                        coarse_affinity_mask - _min) / (_max - _min + 1e-8)
                    coarse_affinity_mask = coarse_affinity_mask.unsqueeze(1)

                    _size = int(A_ss.size()[-1] ** 0.5)
                    all_affinity_masks.append(einops.rearrange(
                        coarse_affinity_mask, "b c (h w)-> b c h w", h=_size, w=_size))
